...
# # 單字聯想工具
# @mcp.tool()
# def word_association(word: str, level: str = "N5") -> str:
#     """聯想單字"""

#     # 預設prompt
#     strDetails = f"請根據以下要求，對「{word}」進行聯想，並分為三大部分:\n\n"
#     strDetails += f"1. 例句: 該單字三句生活化例子(有漢字的話，要附上假名，還有要有中文翻譯)。\n\n"
#     strDetails += f"2. 聯想單字: 請列出與該單字相關的同義詞、反義詞等等，有相關性的單字即可，還有顯示對應詞性。我希望但單字是先顯示中文再顯示對應語言。然後附上一句相關日文例句並附上中文翻譯，越生活化越好。\n\n"
#     strDetails += f"3. 以上內容都要顯示對應單字的語言，顯示對應的單字或句子文法，「{level}」左右程度即可。如果是日文漢字的話，要額外顯示平假名或片假名不然有些漢字不會念。\n\n"
#     strDetails += f"最後，最重要的一點！！！請控制在400字元內，需要上傳至threads，給他人觀看，所以用字遣詞也要注意！！！\n\n"
...
# 日文歌曲主題延伸
@mcp.tool()
def japanese_song_rensou(song: str, composer: str, level: str = "N4、N5") -> str:
    prompt = f"""
    你是我的日文學習夥伴，請根據以下歌曲名稱與創作者，深入理解「歌詞內容」與其中的主題情感，並延伸出適合日文初學者的語言學習素材。

    ⚠️ 請特別注意：請務必根據「實際歌詞」的描寫與語句來發展內容，不要僅根據歌名或音樂風格做模糊猜測。你可以描述歌詞中提到的具體場景（如：夜晚、街燈、散步、車窗倒影等），來說明主題氛圍。

    請依照以下格式整理內容，總字數控制在 400 字內，語氣自然、溫柔，有畫面感，適合日文學習者在社群平台閱讀與分享：

    1. 歌曲主題解析（約 80～100 字）
    根據歌詞，簡要說明歌曲傳達的情感與場景，例如：對時間流逝的感受、孤獨與青春的懷舊、都市夜晚的情緒等。請使用有畫面感的語句，避免空泛形容詞。
    ...
    - 日文（含假名）
    - 詞性（名詞、い形容詞、な形容詞、動詞等）
    - 一句自然、生活化的日文例句（適合 {level}）
    - 中文翻譯

    3. 文法說明（{level} 程度）
    說明例句中出現的文法點，必須與例句完全對應。內容應包含：助詞、常見句型、形容詞變化、て形、～たい形等。

    ✅ 自我檢查：
    - 詞性標註是否正確（如：大人っぽい為い形容詞）
    - 文法說明與例句是否完全對應、無邏輯錯誤
    - 例句是否自然、非直翻，貼近日常生活
...
#     strDetails += f"1. 單字: 為「{level}」會考的單字，提供對應的「{level}等級的例句」(要附上假名)好讓人聯想，並且用於文法教學時一併舉例使用。\n\n"
